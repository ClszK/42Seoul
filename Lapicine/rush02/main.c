...
int				dict_parsing_arr(t_numset *head, char **t_under, char *file);
...
int	str_is_correct(t_numset *head, char **t_under, char **str)
{
	int	 num_len;
...
int     num_split(t_numset *head, char *str)
{
    ...
    if (3 - (str_len % 3) != 3)
	{
		node->val = ft_atoi(ft_strncpy(num, str, 3 - (str_len % 3)));
		str += 3 - (str_len % 3);
		node = node->next;
	}
    while (node->next && *(str))
...
int	main(int argc, char *argv[])
{
	...
	if (ft_str_is_numeric(argv[argc - 1]) == 0 || argc > 3)
	{
		write(1, "Error\n", 7);
		return (-1);
	}
	...
	if (dict_parsing_arr(head, map_three_less, file) == 0)
	{
		printf("Dict Error\n");
		return (-1);
	}
	print_set(ft_atoi(argv[1]), map_three_less);
	return (0);
}