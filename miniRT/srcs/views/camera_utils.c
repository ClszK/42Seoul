...
/*   Updated: 2024/04/21 10:16:31 by jeholee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera	camera_init(t_canvas cvs, t_point3 center, t_vec3 cam_dir)
{
	...
	scale_dir = vec3_mul_scal(&cam_dir, cvs.focal_length);
	cam.delta_u = vec3_div_scal(&cvs.viewport_u, cvs.image_width);
	cam.delta_v = vec3_div_scal(&cvs.viewport_v, cvs.image_height);
	upper_left = vec3_init(\
		center.x - cvs.viewport_u.x / 2 - cvs.viewport_v.x / 2 - scale_dir.x \
		center.y - cvs.viewport_u.y / 2 - cvs.viewport_v.y / 2 - scale_dir.y \
		center.z - cvs.viewport_u.z / 2 - cvs.viewport_v.z / 2 - scale_dir.z);
