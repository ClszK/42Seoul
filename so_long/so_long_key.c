...
/*   Updated: 2023/09/24 00:10:47 by ljh              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void process_and_put_image(void *mlx, void *win, void *img, int x_offset, int y_offset)
{
    ...
    for (int y = 0; y < 64; y++)
    {
        for (int x = 0; x < 64; x++)
        {
            ...
            {
                mlx_pixel_put(mlx, win, x + x_offset, y + y_offset, pixel_data);
            }
        }
    }
}
...
int	key_hook(int keycode, t_game *game)
{
	...
	else if (keycode == KEY_W && m_cfg->map[m_cfg->pos_y - 1][m_cfg->pos_x] != '1')
	{
		...
		process_and_put_image(game->mlx_ptr, game->win_ptr, img_ptr, WIDTH * m_cfg->pos_x, HEIGHT * m_cfg->pos_y - 32);
        ...
        // 원본에서 픽셀 데이터를 가져옴
				...
				printf("%u ", pixel_data);
        // 대상 위치에 픽셀 데이터를 설정
        		// *(unsigned int *)(dst_data + pixel_pos) = pixel_data;
    		}
			ft_printf("\n");
		}
