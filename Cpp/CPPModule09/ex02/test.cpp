...
void insert(std::vector<std::pair<int, int> >& mainChain,
  ...
  for (int mainIdx = 0; mainIdx < mainChain.size(); ++mainIdx) {
    if (mainChain[mainIdx].second != mainIdx) {
      for (int pendingPos = mainIdx; pendingPos < pendingChain.size(); pendingPos += mainChain.size())
        std::swap(pendingChain[pendingPos], pendingChain[mainChain[pendingPos].second]);
    }
  }
  ...
  for (int i = 0; i < jacobsthal.size(); ++i) {
    int index =
        (pendingSize <= jacobsthal[i] ? pendingSize - existOdd
                                              : jacobsthal[i]);
    ...
    for (int j = index - 1; j >= prevIdx; --j) {
      ...
      std::pair<int, int> temp = std::make_pair(pendingChain[j], it - mainChain.begin());
